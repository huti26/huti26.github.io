...
function chance_calc(level, tier, gold, champ_picked, other_champs_picked) {
  ...
  amount_of_all =
    unique_champs(tier) * champ_pool(tier) - other_champs_picked - champ_picked;
  hit_chance_per_throw =
    throw_chance(level, tier) * (amount_of_hits / amount_of_all);
...
function tftUnluckCalc() {
  ...
  var other_champs_picked = document.getElementById("other_champs_picked")
    .value;
  ...
  var max_other_champs_picked =
    champ_pool(tier) * unique_champs(tier) - champ_picked;
  if (
    other_champs_picked < 0 ||
    other_champs_picked > max_other_champs_picked
  ) {
    alert(
